...
export const safariRouter = createTRPCRouter({
    ...
    getAllSafaris: publicProcedure
        ...
        .query(async ({ input }) => {
            try {
                const response = await axios.get<ApiResponseProps<SafariPaginationProps>>(`${env.API_URL}/api/v1/safari/all?limit=${input.limit}&cursor=${input.cursor ?? 0}`);
    ...
    getSafariDetail: publicProcedure
        ...
        .query(async ({ input }) => {
            try {
                const response = await axios.get<ApiResponseProps<SafariDetailProps>>(`${env.API_URL}/api/v1/safari/${input.safariId}`);
                return response.data.data
            } catch (error) {
                if (error instanceof TRPCClientError) {
                    console.error(error.message)
